...
--
-- The legacy config wrapped everything in a "global" submap entered on startup.
-- That submap was vestigial (no other submaps, no reset) and triggered a
-- Hyprland 0.55.3 bug: mouse-button and scroll events don't carry the active
-- submap, so any mouse bind inside a submap never fired. Defining the binds at
-- top level (default submap) is functionally identical and avoids the bug.
...
-- Mouse move/resize. window.drag()/resize() self-handle press+release, so no
-- option flag is needed.
